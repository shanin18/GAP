...
- **Route transitions feel like screen transitions**, not page loads — slide/fade between Home → Country → Services using View Transitions API where supported, so moving through the site feels like navigating app screens, not clicking links
...
This is what actually sells the "app, not a website" feeling on top of the layout choices above — motion is what a screenshot can't show. Concrete inventory, all driven by **Custom Animation** for anything JS-orchestrated and plain CSS `transition`/`@keyframes` for simple state changes (cheaper, no JS needed):
