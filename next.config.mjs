...
const imageHosts = (process.env.IMAGE_HOSTS ?? '').split(',').map((h) => h.trim()).filter(Boolean);
if (!imageHosts.length && process.env.NODE_ENV === 'production') {
  console.warn('[next.config] IMAGE_HOSTS is not set: remote images are allowed from ANY host.');
}
