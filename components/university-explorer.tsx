...
export function UniversityExplorer({
  ...
  universities: CmsUniversity[];
}) {
  const t = useWebsiteContent("university-filters");
    ...
    () =>
      Array.from(
        new Set(
          universities
            .map((u) =>
              typeof u.country === "object" ? u.country?.name : undefined,
            )
  ...
  const filtered = useMemo(
    () =>
      universities.filter((u) => {
        const countryName =
          typeof u.country === "object" ? u.country?.name : "";
        const haystack =
          `${u.name} ${u.city ?? ""} ${countryName ?? ""}`.toLowerCase();
        return (
    ...
    <div>
      <div className="grid gap-3 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-3 sm:grid-cols-[1fr_auto]">
        <label className="flex min-h-12 items-center gap-3 rounded-xl bg-[var(--background)] px-4">
          ...
          <Input
            ...
            placeholder={t("Search university or city")}
      ...
      {filtered.length ? (
        ...
        <div className="mt-8">
          <EmptyState
            icon={<Search size={24} />}
            title={t("No universities match your search.")}
            description={t(
              "Try another name or destination, or clear the filters to see all available universities.",
            )}
          >
            <Button
              type="button"
