...
export function UniversityExplorer({ universities }: { universities: CmsUniversity[] }) {
  ...
  return <div>
    <div className="grid gap-3 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-3 sm:grid-cols-[1fr_auto]">
    ...
    {filtered.length ? <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((u) => <UniversityCard key={u.id ?? u.slug} university={u}/>)}</div> :
      <div className="mt-8"><EmptyState icon={<Search size={24} />} title="No universities match your search." description="Try another name or destination, or clear the filters to see all available universities."><Button type="button" variant="outline" onClick={() => { setQuery(''); setCountry('all'); }}>Clear filters</Button></EmptyState></div>}
  </div>;
}
