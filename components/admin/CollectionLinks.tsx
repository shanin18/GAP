...
export async function CollectionLinks({
    ...
    .map(({ slug, labels }) => ({
      ...
      label:
        typeof labels.plural === "function"
          ? slug
          : (getTranslation(labels.plural, i18n) as string),
  ...
  const [newLeads, newApplications] = await Promise.all([
    visible.some((c) => c.slug === "leads")
      ? countOf(payload, "leads", { status: { equals: "new" } }, user)
      : 0,
  ...
  return (
    <CollectionLinksClient
      dashboardHref={payload.config.routes.admin}
