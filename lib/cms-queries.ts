...
export const getTestimonials = cache(async (limit = 6, countrySlug?: string): Promise<CmsTestimonial[]> => {
  try {
    const payload = await getCms();
    const where: Where | undefined = countrySlug
      ? { or: [{ 'country.slug': { equals: countrySlug } }, { country: { exists: false } }] }
      : undefined;
    // Visitors only get published testimonials: the collection's read rule adds that filter
    const result = await payload.find({ overrideAccess: false, collection: 'testimonials', where, sort: 'sortOrder', limit, depth: 1 });
...
export const getNewsBySlug = cache(async (slug: string): Promise<CmsNews | null> => {
  try {
    const payload = await getCms();
    const result = await payload.find({
      overrideAccess: false,
...
export const getCountryBySlug = cache(async (slug: string): Promise<CmsCountry | null> => {
  try {
    const payload = await getCms();
    const result = await payload.find({
      overrideAccess: false,
...
export async function getUniversities(options?: { countrySlug?: string; featured?: boolean; limit?: number }): Promise<CmsUniversity[]> {
  try {
    ...
    if (options?.countrySlug) conditions.push({ 'country.slug': { equals: options.countrySlug } });
...
export const getUniversityBySlug = cache(async (slug: string): Promise<CmsUniversity | null> => {
  try {
    const payload = await getCms();
    const result = await payload.find({
      overrideAccess: false,
...
export const getApplicationOptions = cache(async () => {
  ...
  const [countries, universities] = await Promise.all([
    payload.find({
      collection: 'countries', overrideAccess: false, pagination: false, depth: 0, sort: 'name',
      select: { name: true, slug: true },
    }),
    payload.find({
      collection: 'universities', overrideAccess: false, pagination: false, depth: 0, sort: 'name',
      where: { status: { equals: 'published' } },
      select: { name: true, country: true },
    }),
  ]);
  return {
    countries: countries.docs.map((c) => ({ id: c.id, name: c.name, slug: c.slug })),
