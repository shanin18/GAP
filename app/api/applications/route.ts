...
export const { GET, PATCH, DELETE, PUT, OPTIONS } =
  payloadCollectionRoutes("applications");
...
export async function POST(request: Request) {
  try {
    const guard = await guardPublicPost(request, {
      key: "application",
      limit: 3,
      windowMs: 15 * 60_000,
    });
    ...
    if (isHoneypotFilled(guard.body)) {
      return NextResponse.json(
        { ok: true, reference: makeReference() },
        { status: 201 },
      );
    }
    ...
    if (data.universityId) {
      ...
      if (!universities.docs.length) {
        return NextResponse.json(
          {
            error:
              "Please select a published university in your chosen destination.",
          },
          { status: 400 },
        );
      }
    ...
    const recent = await payload.count({
      ...
      where: {
        and: [
          ...
          {
            createdAt: {
              greater_than: new Date(Date.now() - 10 * 60_000).toISOString(),
            },
          },
        ],
      },
    ...
    if (recent.totalDocs > 0) {
      return NextResponse.json(
        {
          error:
            "We have already received your application. Our team will be in touch soon.",
        },
        { status: 409 },
      );
    }
    ...
    const application = await payload.create({
      ...
      data: {
        ...
        documents: [
          ...
          {
            label: "English language evidence (if applicable)",
            status: "required",
          },
        ],
      },
    ...
    return NextResponse.json(
      { ok: true, reference: application.reference },
      { status: 201 },
    );
  } catch (error) {
