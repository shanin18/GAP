...
export default async function CountryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader/><main>
    <section className="country-hero"><div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <p className="eyebrow">Study destination</p><h1 className="display-xl mt-4">Study in <em>{country.name}.</em></h1>
      <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--muted)]">{!cmsCountry && fallback ? fallback.intro : 'Discover study options, admission guidance, and university pathways for this destination.'}</p>
      <ApplyNowDialog triggerClass="mt-8 rounded-full bg-[var(--primary)] px-6 py-3 font-semibold text-white"/>
    </div></section>
    <section><div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div className="grid gap-5 md:grid-cols-3">{facts.map((fact, i) => <article className="rounded-[1.5rem] border border-[var(--border)] p-7" key={fact}><span className="text-sm text-[var(--muted)]">0{i+1}</span><h2 className="mt-10 font-display text-2xl">{fact}</h2></article>)}</div>
      <div className="mt-20 grid gap-12 lg:grid-cols-[1fr_.8fr]"><div><p className="eyebrow">Why this destination</p><h2 className="mt-3 font-display text-4xl">Choose with your goals in mind.</h2><p className="mt-5 max-w-2xl leading-8 text-[var(--muted)]">GAP combines destination guidance, university selection, and application support into one practical journey.</p></div>
        <div className="rounded-[2rem] bg-[var(--surface)] p-8"><GraduationCap size={28} className="text-[var(--primary)]"/><h3 className="mt-7 font-display text-2xl">Partner universities</h3><p className="mt-3 text-[var(--muted)]">Related universities will be managed through the CMS.</p><a href="/services" className="mt-6 inline-flex items-center gap-2 font-semibold">Explore support <ArrowRight size={17}/></a></div>
      </div>
    </div></section>
    {universities.length > 0 && <section className="border-t border-[var(--border)]"><div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24"><p className="eyebrow">University options</p><div className="mt-3 flex flex-wrap items-end justify-between gap-5"><h2 className="font-display text-4xl">Explore universities in {country.name}</h2><a href="/universities" className="font-semibold text-[var(--primary)]">View all universities</a></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{universities.map((university) => <UniversityCard key={university.id ?? university.slug} university={university}/>)}</div></div></section>}
