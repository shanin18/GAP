...
export default async function UniversityPage({
  ...
  const t = await getSectionText("university-page");

  const { slug } = await params;
  const university = await getUniversityBySlug(slug);
  if (!university) notFound();
  const country =
    typeof university.country === "object" ? university.country : undefined;
  const highlights =
    university.highlights?.map((item) => item.text).filter(Boolean) ?? [];
  ...
  return (
    <>
      ...
      <main>
        <section className="border-b border-[var(--border)]">
          <div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24">
            ...
            <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_.45fr] lg:items-end">
              <div>
                <div className="flex size-20 items-center justify-center overflow-hidden rounded-3xl bg-[var(--surface)]">
                  {university.logoUrl ? (
                    <img
                ...
                <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-primary mt-8">
              ...
              <div className="flex flex-wrap gap-3 lg:justify-end">
                ...
                {university.websiteUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] px-6 py-3 font-semibold"
                  >
        ...
        <section>
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[1.2fr_.8fr] lg:px-8 lg:py-24">
            <div>
            ...
            <aside className="rounded-3xl bg-[var(--surface)] p-8">
              <h2 className="font-display text-2xl">
                {t("Why consider this university?")}
              </h2>
              <div className="mt-6 space-y-4">
                {(highlights.length
                  ? highlights
                  : [
                      "Discuss suitable programs with a GAP counsellor",
                      "Review entry requirements for your profile",
              ...
              {country?.slug && (
                <Link
                  ...
                  className="mt-8 inline-flex font-semibold text-[var(--primary)]"
                >
                  {t("Explore studying in ")}
                  {country.name}
                </Link>
              )}
            </aside>
