...
export default async function UniversityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader/><main>
    <section className="border-b border-[var(--border)]"><div className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24">
      <Link href="/universities" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--muted)]"><ArrowLeft size={16}/> All universities</Link>
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_.45fr] lg:items-end">
        <div><div className="flex size-20 items-center justify-center overflow-hidden rounded-[1.5rem] bg-[var(--surface)]">{university.logoUrl ? <img src={university.logoUrl} alt="" className="h-full w-full object-contain p-3"/> : <span className="font-display text-2xl">{university.name.slice(0,2).toUpperCase()}</span>}</div>
          <p className="eyebrow mt-8">University profile</p><h1 className="display-xl mt-4">{university.name}</h1>
          <p className="mt-5 flex items-center gap-2 text-[var(--muted)]"><MapPin size={18}/>{[university.city, country?.name].filter(Boolean).join(', ')}</p>
        </div>
        <div className="flex flex-wrap gap-3 lg:justify-end"><ApplyNowDialog triggerClass="rounded-full bg-[var(--primary)] px-6 py-3 font-semibold text-white"/>
    ...
    <section><div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[1.2fr_.8fr] lg:px-8 lg:py-24">
      <div><p className="eyebrow">Overview</p><h2 className="mt-3 font-display text-4xl">About {university.name}</h2><p className="mt-6 max-w-3xl whitespace-pre-line text-lg leading-8 text-[var(--muted)]">{university.description}</p></div>
      <aside className="rounded-[2rem] bg-[var(--surface)] p-8"><h2 className="font-display text-2xl">Why consider this university?</h2>
