...
export default async function ArticlePage({params}:{params:Promise<{slug:string}>}){
  ...
  return <><SiteHeader/><main><article>
    <header className="border-b border-[var(--border)]"><div className="mx-auto max-w-4xl px-5 py-16 lg:px-8 lg:py-24"><Link href="/news" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--muted)]"><ArrowLeft size={16}/> Back to journal</Link><p className="eyebrow mt-10">GAP Journal</p><h1 className="mt-4 font-display text-5xl leading-[1.02] sm:text-6xl">{article.title}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--muted)]">{article.shortBlurb}</p><time className="mt-7 block text-sm text-[var(--muted)]">{new Date(article.publishedDate).toLocaleDateString('en',{month:'long',day:'numeric',year:'numeric'})}</time></div></header>
    {article.coverImageUrl ? <div className="mx-auto max-w-6xl px-5 pt-10 lg:px-8"><img src={article.coverImageUrl} alt="" className="aspect-[16/8] w-full rounded-[2rem] object-cover"/></div> : null}
    <div className="mx-auto grid max-w-4xl gap-12 px-5 py-14 lg:px-8 lg:py-20"><div className="article-copy"><p>{body || article.shortBlurb}</p></div><aside className="rounded-[2rem] bg-[var(--primary)] p-8 text-white sm:p-10"><p className="text-sm font-bold uppercase tracking-[.15em] text-white/60">Need personal guidance?</p><h2 className="mt-3 font-display text-3xl">Turn your research into a clear application plan.</h2><ApplyNowDialog triggerClass="mt-6 rounded-full bg-white px-6 py-3 font-semibold text-[var(--primary)]"/></aside></div>
